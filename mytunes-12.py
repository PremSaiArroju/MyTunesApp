...
class MyTunesApp:
    ...
    def create_widgets(self):
        ...
        self.song_treeview = ttk.Treeview(self.frame_top, columns=columns, show='headings', style="Custom.Treeview")
        ...
        self.frame_controls = tk.Frame(self.frame_bottom)
        self.frame_controls.pack(pady=10)

        # Buttons for control
        self.btn_play = tk.Button(self.frame_controls, text="Play", command=self.play_song)
        self.btn_play.grid(row=0, column=0, padx=5)

        self.btn_pause = tk.Button(self.frame_controls, text="Pause", command=self.pause_song)
        self.btn_pause.grid(row=0, column=1, padx=5)

        self.btn_unpause = tk.Button(self.frame_controls, text="Unpause", command=self.unpause_song)
        self.btn_unpause.grid(row=0, column=2, padx=5)

        self.btn_stop = tk.Button(self.frame_controls, text="Stop", command=self.stop_song)
        self.btn_stop.grid(row=0, column=3, padx=5)

        self.btn_prev = tk.Button(self.frame_controls, text="Prev", command=self.prev_song)
        self.btn_prev.grid(row=0, column=4, padx=5)

        self.btn_next = tk.Button(self.frame_controls, text="Next", command=self.next_song)
        self.btn_next.grid(row=0, column=5, padx=5)

        # Frame for add/delete buttons
        self.frame_add_delete = tk.Frame(self.frame_bottom)
        self.frame_add_delete.pack(pady=10)

        self.btn_add = tk.Button(self.frame_add_delete, text="Add", command=self.add_song)
        self.btn_add.grid(row=0, column=0, padx=5)

        self.btn_delete = tk.Button(self.frame_add_delete, text="Delete", command=self.delete_song)
        ...
        # Add a custom style for the Treeview
        style = ttk.Style()
